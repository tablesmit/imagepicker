...
YAHOO.ip.Controller.prototype = {
	...
	init : function() {
		...
		var prefs = Cc["@mozilla.org/preferences-service;1"]
				...
				.getBranch("extensions.imagepicker.");

		// init image grid
		var gridSize = window.innerWidth - 6;

		var thumbnailType = prefs.getCharPref("displayrule.thumbnailType");
		this.imageGrid = new YAHOO.ip.ImageGrid("imageContainer", gridSize,
				thumbnailType, false, false, false);
	},
	...
	loadPickWindow : function() {
		...
		document.getElementById("minWidthTB").value = prefs
				.getIntPref("filter.minWidth");
		document.getElementById("minHeightTB").value = prefs
				.getIntPref("filter.minHeight");
		document.getElementById("minSizeTB").value = prefs
				.getIntPref("filter.minSize");

		document.getElementById("imageTypeBmpCB").checked = !prefs
				.getBoolPref("filter.skipImageTypes.bmp");
		document.getElementById("imageTypeJpegCB").checked = !prefs
				.getBoolPref("filter.skipImageTypes.jpg");
		document.getElementById("imageTypePngCB").checked = !prefs
				.getBoolPref("filter.skipImageTypes.png") == true;
		document.getElementById("imageTypeGifCB").checked = !prefs
				.getBoolPref("filter.skipImageTypes.gif");

		var thumbnailType = prefs.getCharPref("displayrule.thumbnailType");
		switch (thumbnailType) {
			case 'small' :
				document.getElementById("thumbnailTypeSmallMI").setAttribute(
						"checked", true);
				break;
			case 'normal' :
				document.getElementById("thumbnailTypeNormalMI").setAttribute(
						"checked", true);
				break;
			case 'large' :
				document.getElementById("thumbnailTypeLargeMI").setAttribute(
						"checked", true);
				break;
			default :
				YAHOO.ip.Logger.info("gDisplayRule.thumbnailType = "
						+ thumbnailType);
		}

		var saveFolderPath = YAHOO.ip.PrefUtils.getUnicodeChar(prefs,
				"savedFolderPath");
	...
	unloadPickWindow : function() {
		...
		// save display rule
		prefs.setCharPref("displayrule.thumbnailType",
				this.imageGrid.thumbnailType);

		// save filter
		prefs.setIntPref("filter.minWidth", document
						.getElementById("minWidthTB").value);
		prefs.setIntPref("filter.minHeight", document
						.getElementById("minHeightTB").value);
		prefs.setIntPref("filter.minSize",
				document.getElementById("minSizeTB").value);

		prefs.setBoolPref("filter.skipImageTypes.jpg", !document
						.getElementById("imageTypeJpegCB").checked);
		prefs.setBoolPref("filter.skipImageTypes.png", !document
						.getElementById("imageTypePngCB").checked);
		prefs.setBoolPref("filter.skipImageTypes.bmp", !document
						.getElementById("imageTypeBmpCB").checked);
		prefs.setBoolPref("filter.skipImageTypes.gif", !document
						.getElementById("imageTypeGifCB").checked);

		// save saved folder
		YAHOO.ip.PrefUtils.setUnicodeChar(prefs, "savedFolderPath", document
						.getElementById("browsedirTB").value);
	},
	...
	doFilter : function() {
		...
		this.filter = new YAHOO.ip.Filter(sizeFilter, widthFilter,
				heightFilter, skipImageTypeFilter);
		...
		document.getElementById("filterStat").label = "Current: "
				+ newImageConut + ", Total: " + oldImageConut + ", Filter:"
				+ (oldImageConut - newImageConut);
	...
	browseDir : function() {
		...
		filePicker.init(window, 'Select Save Folder',
	...
	doSaveImages : function() {
		...
		if (dest) {
			...
			try {
				...
				subFolderName = YAHOO.ip.FileUtils
						.toValidDirectoryName(window.document.title);
				...
				alert("Cannot create subfolder! subFolderName = "
						+ subFolderName + ",e = " + e);
			}
			...
			for (var i = 0; i < this.imageList.length; i++) {
				document.getElementById("filterStat").label = "Saving"
						+ this.imageList[i].fileName;

				// Set default file ext as jpg
				if (this.imageList[i].fileExt == null
						|| this.imageList[i].fileExt == "") {
					this.imageList[i].fileName = this.imageList[i].fileName
							+ ".jpg";
				}
				var file = YAHOO.ip.FileUtils.createUniqueFile(
						this.imageList[i].fileName, dest, fileNames);
	...
	saveImageToFile : function(imageInfo, file) {
		...
		var cacheKey = Cc['@mozilla.org/supports-string;1']
		...
		try {
			...
			YAHOO.ip.Logger.info("cannot save file size for URL: "
					+ imageInfo.url + ", exception = " + e);
		}
	},
	...
	saveFileByDownloadManager : function(fromURL, toFile) {
		...
		// Start download
		var dl = dm.addDownload(dm.DOWNLOAD_TYPE_DOWNLOAD, fromURI, toURI,
				toFile.leafName, mime, Math.round(Date.now() * 1000), null,
				pers);
...
YAHOO.ip.DownloadProgressListener.prototype = {
	...
	onProgressChange : function(aWebProgress, aRequest, aCurSelfProgress,
		...
		document.getElementById("downloadMeter").value = (aCurTotalProgress / aMaxTotalProgress)
				* 100;
		var info = aDownload.displayName + ", "
				+ (aCurSelfProgress / aMaxSelfProgress) * 100 + "%";
